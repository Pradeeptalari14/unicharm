                ...
                <div className="bg-white p-4 md:p-6 rounded-lg shadow-sm border border-gray-200">
                    ...
                    <div className="md:hidden space-y-4">
                        {users
                            ...
                            .map(user => (
                                <div key={user.id} className={`bg-white border rounded-lg p-4 shadow-sm ${!user.isApproved ? 'border-orange-200 bg-orange-50/30' : 'border-gray-200'}`}>
                                    <div className="flex justify-between items-start mb-3">
                                        <div>
                                            <div className="font-bold text-gray-800">{user.fullName || user.username}</div>
                                            <div className="text-xs text-gray-500">{user.email}</div>
                                        </div>
                                        {user.isApproved ? (
                                            <span className="px-2 py-1 rounded-full text-xs font-bold bg-green-100 text-green-700 flex items-center gap-1"><Check size={10} /> Active</span>
                                        ) : (
                                            <span className="px-2 py-1 rounded-full text-xs font-bold bg-orange-100 text-orange-700 flex items-center gap-1"><ShieldAlert size={10} /> Pending</span>
                                        )}
                                    </div>
                                    ...
                                    <div className="flex gap-2">
                                        {!user.isApproved ? (
                                            <>
                                                <button onClick={(e) => handleApprove(e, user.id)} className="flex-1 bg-green-600 text-white py-2 rounded font-bold text-xs shadow-sm">Approve</button>
                                                <button onClick={(e) => handleReject(e, user.id)} className="flex-1 bg-red-600 text-white py-2 rounded font-bold text-xs shadow-sm">Reject</button>
                                            </>
                                        ) : (
                                            <button onClick={(e) => openResetPassword(e, user)} className="flex-1 bg-white border border-gray-300 text-gray-600 py-2 rounded font-bold text-xs shadow-sm flex justify-center items-center gap-2 hover:bg-gray-50">
        ...
        return (
            <div className="space-y-6">
                <div className="bg-white p-4 md:p-6 rounded-lg shadow-sm border border-gray-200">
                    ...
                    {/* Mobile Card View for Sheets */}
                    <div className="md:hidden space-y-4">
                        {sheets
                            .filter(s => s.id.includes(searchTerm) || s.supervisorName.toLowerCase().includes(searchTerm.toLowerCase()))
                            .map(s => (
                                <div key={s.id} onClick={() => onViewSheet(s)} className="bg-white border border-slate-200 rounded-lg p-4 shadow-sm active:scale-[0.99] transition-transform">
                                    <div className="flex justify-between items-center mb-3">
                                        <span className="font-mono text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded">{s.id}</span>
                                        <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${s.status === 'LOCKED' ? 'bg-orange-100 text-orange-700' :
                                            s.status === 'COMPLETED' ? 'bg-green-100 text-green-700' :
                                                'bg-slate-100 text-slate-600'
                                            }`}>{s.status}</span>
                                    </div>
                                    <div className="grid grid-cols-2 gap-y-2 text-sm text-slate-600 mb-3">
                                        <div><span className="text-slate-400 text-xs block uppercase">Date</span>{s.date}</div>
                                        <div><span className="text-slate-400 text-xs block uppercase">Supervisor</span>{s.supervisorName.split(' ')[0]}</div>
                                        <div className="col-span-2"><span className="text-slate-400 text-xs block uppercase">Dock / Destination</span>{s.loadingDockNo || s.destination || '-'}</div>
                                    </div>
                                    <div className="flex justify-end border-t border-slate-100 pt-3">
                                        <button
                                            onClick={(e) => handleDelete(e, s.id)}
                                            className="text-red-500 text-xs font-bold flex items-center gap-1 px-3 py-1.5 bg-red-50 rounded hover:bg-red-100"
                                        >
                        ...
                        {sheets.length === 0 && <div className="text-center text-slate-400 italic p-6">No sheets found.</div>}
                    </div>
                </div>
            </div>
        );
